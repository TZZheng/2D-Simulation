...
int main() {
	...
	int total_h = 1;
	int loop_temp = 0;
	int i,j, k, ii, iii,loop; //for loop
	...
	//double *pos_e = NULL, *pos_h = NULL, *pos_e_temp0 = NULL, *pos_e_temp1 = NULL, *pos_h_temp0 = NULL, *pos_h_temp1 = NULL;
			...
			while (total_carrier<12500 && total_carrier>0) {
				...
				for (j = 0; j<total_e; ++j) {
					if (if_active_e[j]>0) {
						...
						if (pos_e[j][2]>width_z * 1000 || fabs(pos_e[j][0])>width_x * 1000 || fabs(pos_e[j][1]>width_y * 1000)) {
							--total_carrier;
